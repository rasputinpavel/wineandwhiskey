...
Вариант Б: cron / расписание на ежедневный прогон.

**Обогащаем ТОЛЬКО in-stock продукты** — то, что реально показывается на витрине. Out-of-stock товары не отображаются, тратить на них Vivino-поиск бессмысленно. Селектор фильтрует через `.gt("stock_quantity", 0)` — поправь имя колонки под свою схему (может быть `in_stock` boolean или `quantity` int).
