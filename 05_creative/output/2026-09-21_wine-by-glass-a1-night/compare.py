...
DATE = "2026-09-21"
KEYS = tuple(sys.argv[1:]) or ("a", "b", "c", "d", "e")
COL_W = 520
PAD = 26
LABEL_H = 44
...
def main():
    ...
    sheet = Image.new("RGB", (PAD + (COL_W + PAD) * len(cols), PAD + (h + LABEL_H + PAD) * 3), "#1b1917")
    ...
    out = HERE / f"compare_day-night_{''.join(KEYS)}_{DATE}.png"
    sheet.save(out)
