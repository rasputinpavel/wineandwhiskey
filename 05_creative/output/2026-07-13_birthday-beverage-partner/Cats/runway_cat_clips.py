...
OUT = os.path.join(HERE, "assets", "runway")
...
def data_uri(scene_png):
    ...
    """
    tmp = os.path.join(OUT, "_in_" + os.path.basename(scene_png).replace(".png", ".jpg"))
    subprocess.run([
...
def submit(key, model, scene):
    body = {
        "model": model,
        "promptImage": data_uri(os.path.join(SCENES, f"{scene}.png")),
...
def main():
    ...
    for s, tid in tasks.items():
        out_path = os.path.join(OUT, f"{s}.mp4")
        try:
            url = poll(key, tid)
