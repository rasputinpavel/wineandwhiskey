...
async function generateVideoBackground(briefId: string, brief: Record<string, unknown>) {
  const prompts = brief.visual_prompts as Array<{ scene: string; prompt_en: string; duration_s: number }>
  ...
  const videoUrl = await generateVideo(briefId, prompts, textCopy)
