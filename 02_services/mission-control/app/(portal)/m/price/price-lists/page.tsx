...
export default function PriceListsPage() {
  ...
  function startPoll(id: string) {
    ...
    pollRef.current[id] = setInterval(async () => {
      const res = await fetch(`/api/m/price/vivino/status?price_list_id=${id}`)
      if (!res.ok) return
    ...
    load().then(async all => {
      await Promise.all(
        all.filter(pl => pl.status === 'done' && pl.item_count > 0).map(async pl => {
          const sr = await fetch(`/api/m/price/vivino/status?price_list_id=${pl.id}`)
          if (!sr.ok) return
  ...
  async function handleEnrich(id: string, mode: 'missing' | 'failed' | 'all' = 'missing') {
    ...
    const res = await fetch('/api/m/price/vivino/enrich', {
      method: 'POST',
  ...
  async function handlePause(id: string) {
    ...
    await fetch(`/api/m/price/vivino/jobs/${jobId}`, {
  ...
  async function handleResume(id: string) {
    const jobId = enrichStatus[id]?.jobId
    if (!jobId) return
    const res = await fetch(`/api/m/price/vivino/jobs/${jobId}`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  async function handleDelete(id: string, name: string) {
    ...
    setDeleting(id)
    await fetch(`/api/m/price/price-lists/${id}`, { method: 'DELETE' })
                  ...
                  {es.state === 'starting' && (
                    <>
                      ...
                      <button
                        ...
                        className="p-1.5 text-xs text-graphite bg-cream hover:bg-pale-stone/30 rounded-lg transition-colors"
