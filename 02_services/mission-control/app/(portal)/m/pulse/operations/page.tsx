...
export default async function PulseOperationsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return (
    <>
      <div className="flex items-baseline justify-between mb-3 flex-wrap gap-3">
        <h2 className="font-heading text-xl text-deep-black">Operations · {range.label}</h2>
      ...
      <div className="flex gap-1 text-xs mb-4">
        {PERIODS.map(p => {
          ...
          return (
            <Link key={p.key}
              href={qs ? `/m/pulse/operations?${qs}` : '/m/pulse/operations'}
