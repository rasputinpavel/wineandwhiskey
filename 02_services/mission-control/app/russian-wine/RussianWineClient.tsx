...
const T = {
  ...
  priceNote: { ru: 'Цены магазина, ฿ за бутылку', en: 'Store prices, ฿ per bottle' },
  outroTitle: { ru: 'Ждём вас в магазине', en: 'See you at the store' },
  outroLead: {
    ru: 'Полный ассортимент русских и мировых вин и крепких напитков — на Раваи, Пхукет. Открыты ежедневно 11:00–22:00.',
...
// Clean bottle-silhouette placeholder for SKUs without a product photo yet.
function BottlePlaceholder({ label }: { label: string }) {
  return (
    <div className="flex flex-col items-center justify-center gap-3 text-pale-stone">
      <svg width="42" height="104" viewBox="0 0 42 104" fill="none" aria-hidden>
        <path d="M17 3h8v17c0 2.5 7 6 7 15v60a5 5 0 0 1-5 5H15a5 5 0 0 1-5-5V35c0-9 7-12.5 7-15V3z"
          stroke="currentColor" strokeWidth="1.6" />
        <path d="M10 52h22" stroke="currentColor" strokeWidth="1.2" opacity="0.6" />
      </svg>
      <span className="overline text-graphite/45">{label}</span>
    </div>
  )
}

function fmtPrice(p?: number) {
  return p == null ? null : '฿' + p.toLocaleString('en-US')
}

function BottleCard({ b, lang, price }: { b: Bottle; lang: Lang; price?: number }) {
  ...
  return (
    <div className="group flex flex-col rounded-md bg-warm-white border border-pale-stone/50 shadow-card hover:shadow-card-hover transition-shadow overflow-hidden">
      <div className="relative bg-gradient-to-b from-cream to-pale-stone/40 h-56 flex items-center justify-center p-4">
        ...
        {priceStr && (
          <span className="absolute top-3 right-3 font-heading font-semibold text-sm bg-wine-red text-warm-white px-2.5 py-1 rounded-sm shadow-sm">
            {priceStr}
          </span>
        )}
        {b.image ? (
          ...
          <BottlePlaceholder label={b.producer[lang]} />
        )}
      </div>
...
export default function RussianWineClient({ prices }: { prices: Record<string, number> }) {
  ...
  return (
    <main className="min-h-screen bg-warm-white text-deep-black">
      ...
      <div className="max-w-6xl mx-auto px-5 py-16 sm:py-20">
        <div className="overline text-graphite/60 mb-10">{T.priceNote[lang]}</div>
          ...
          return (
            <section key={cat} className="mb-16">
              ...
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {items.map((b) => <BottleCard key={b.id} b={b} lang={lang} price={prices[b.code]} />)}
              </div>
            </section>
          )
        })}
