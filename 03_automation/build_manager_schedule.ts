...
// Shift-based pay (optional): full-month salary, shifts worked, and the baseline
// (shifts = full pay, normally days-in-month ÷ 2). When all present for a manager,
// the emitted `fixed` is prorated = nominal × shifts / baseline, and the nominal /
// shifts / baseline are recorded so the Bonuses tab can explain the deviation.
const nominal = parseKV(arg("nominal"));
const shifts = parseKV(arg("shifts"));
const baseline = arg("baseline") ? +arg("baseline")! : undefined;
...
async function main() {
  ...
  // Effective fixed salary: prorate by shifts when nominal + shifts + baseline
  // are all supplied for a manager, otherwise use the plain --fixed value.
  const shiftPay = baseline != null && Object.keys(nominal).length > 0 && Object.keys(shifts).length > 0;
  const effectiveFixed = Object.fromEntries(managers.map(m => {
    if (shiftPay && nominal[m] != null && shifts[m] != null) {
      return [m, Math.round(nominal[m] * shifts[m] / baseline! * 100) / 100];
    }
    return [m, fixed[m] ?? 0];
  }));
  ...
  const out: any = {
    ...
    ...(shiftPay ? {
      nominal_fixed: Object.fromEntries(managers.map(m => [m, nominal[m] ?? effectiveFixed[m]])),
      shifts: Object.fromEntries(managers.map(m => [m, shifts[m] ?? baseline])),
      shift_baseline: baseline,
    } : {}),
    ...(prevPaid    ? { paid: prevPaid } : {}),
    ...(prevPaidLog ? { paid_log: prevPaidLog } : {}),
    note: "Auto-generated by build_manager_schedule.ts. B2C uses lib/b2b.ts management classifier (bank-transfer OR name in B2B_PATTERNS → B2B, excluded). Half-day split by Bangkok hour. When nominal/shifts/baseline supplied, fixed = nominal × shifts / baseline (shift-based pay).",
    days,
    remarks,
  };
