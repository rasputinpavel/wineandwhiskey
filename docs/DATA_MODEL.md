...
| `inventory.loyverse_receipt` | table | **Canonical cached receipt mirror** (SALE+REFUND) with `is_b2b` precomputed | `receipt_number` (uniq), `receipt_type`, `is_b2b`, `is_bank_transfer`, `total`, `cost_total` | `sync_loyverse_receipts.ts` (`npm run inv:receipts`) | `lib/dashboard.ts`, `pulse/page.tsx` |
